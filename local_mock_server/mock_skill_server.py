...
def lookup_precomputed(store_path: Path, owner: str, repo: str) -> Dict[str, Any]:
    """Look up an evaluation, preferring hand-curated entries over adapter ones.

    Resolution order:
      1. ``precomputed_evaluations.json`` (hand-curated, paper-final numbers)
      2. ``adapter`` index built from ``Example/<skill>/skill_report.json``
         (live algorithm-side data)
      3. ``not_evaluated``
    """
    key = f"{owner}/{repo}"
    ...
    # 2. Fall back to the adapter index built from real algorithm artifacts.
